...
def read_moves(fd, crates, crate_names):
    ...
    while line:
        ...
        # move the number of crates in the same order
        moved_crates = crates[from_pos][-amount:]
        crates[from_pos] = crates[from_pos][0:-amount]
        crates[to_pos].extend(moved_crates)
        line = fd.readline()
